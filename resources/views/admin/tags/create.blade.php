...
@section('title', 'สร้างแท็กใหม่ · Exclusive')
@section('page-title', 'สร้างแท็กใหม่')

@section('content')
    <form method="POST" action="{{ route('admin.tags.store') }}" class="space-y-4 max-w-lg">
        @csrf
        ...
        <div>
            ...
            <p class="mt-1 text-[11px] text-slate-500">
                ใช้สำหรับ URL friendly เช่น <code>productivity</code>, <code>investment-basic</code>
            </p>
            @error('slug')
                <p class="text-xs text-red-400 mt-1">{{ $message }}</p>
            @enderror
