...
@section('title', 'จัดการหมวดหมู่ · Exclusive')
@section('page-title', 'จัดการหมวดหมู่ (Categories)')
@section('page-subtitle', 'ใช้จัดกลุ่มคอนเทนต์ตามประเภท')

@section('content')
    {{-- หัว + Search --}}
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-4 gap-3">
        <div>
            <h2 class="text-sm font-semibold text-slate-100">หมวดหมู่ทั้งหมด</h2>
            @if(!empty($q))
                <p class="text-xs text-slate-400 mt-1">
                    ค้นหา:
                    <span class="font-semibold text-amber-300">{{ $q }}</span>
                </p>
            @endif
        </div>
    ...
    <div class="flex items-center justify-between mb-4">
        <h3 class="text-sm font-semibold text-slate-100">รายการหมวดหมู่</h3>
    ...
    {{-- Desktop Table --}}
    <div class="hidden md:block rounded-2xl border border-slate-800 bg-slate-900/80 overflow-hidden">
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-slate-800">
                @forelse($categories as $category)
                    <tr>
                        <td class="px-4 py-2">
                            <div class="font-medium text-slate-100">{{ $category->name }}</div>
                        </td>
    ...
    <div class="md:hidden space-y-3">
        @forelse($categories as $category)
            <div class="rounded-xl border border-slate-800 bg-slate-900/80 p-4 space-y-2">
                <div class="font-semibold text-slate-100">
                    {{ $category->name }}
                </div>
                <div class="text-xs text-slate-400">
                    <span class="text-slate-500">Slug: </span>{{ $category->slug }}
                </div>
