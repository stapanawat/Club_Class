    ...
    <div class="space-y-8 relative">
        ...
        <div class="space-y-6">
            ...
            <div class="group">
                ...
                <div
                    class="quill-wrapper rounded-xl border border-slate-700/50 bg-slate-950/50 shadow-inner focus-within:border-indigo-500/50 focus-within:ring-4 focus-within:ring-indigo-500/10 transition-all duration-300">
                    <div id="editor-container" class="text-slate-100">
        ...
        <div class="space-y-6">
            ...
            <div class="grid gap-6 md:grid-cols-2">
                ...
                <div class="group">
                    ...
                    <div
                        class="relative w-full aspect-video rounded-2xl border-2 border-dashed border-slate-700/50 bg-slate-900/30 hover:bg-slate-800/50 transition-all overflow-hidden group-focus-within:border-emerald-500/50 group-focus-within:bg-slate-900/50">

                        {{-- Preview Image --}}
                        <img id="thumbnail-preview"
                            src="{{ !empty($content->thumbnail_url) ? $content->thumbnail_url : '' }}"
                            alt="Thumbnail preview"
                            class="{{ !empty($content->thumbnail_url) ? '' : 'hidden' }} absolute inset-0 w-full h-full object-cover z-0 transition-transform duration-700 group-hover:scale-105">

                        {{-- Placeholder --}}
                        <div id="thumbnail-placeholder"
                            class="{{ !empty($content->thumbnail_url) ? 'hidden' : '' }} absolute inset-0 flex flex-col items-center justify-center text-slate-600 z-0 pointer-events-none">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                                class="w-12 h-12 mb-2 opacity-50 group-hover:text-emerald-500 transition-colors">
                                <path fill-rule="evenodd"
                                    d="M1.5 6a2.25 2.25 0 012.25-2.25h16.5A2.25 2.25 0 0122.5 6v12a2.25 2.25 0 01-2.25 2.25H3.75A2.25 2.25 0 011.5 18V6zM3 16.06V18c0 .414.336.75.75.75h16.5A.75.75 0 0021 18v-1.94l-2.69-2.689a1.5 1.5 0 00-2.12 0l-.88.879.97.97a.75.75 0 11-1.06 1.06l-5.16-5.159a1.5 1.5 0 00-2.12 0L3 16.061zm10.125-7.81a1.125 1.125 0 112.25 0 1.125 1.125 0 01-2.25 0z"
                                    clip-rule="evenodd" />
                            </svg>
                            <span
                                class="text-xs font-medium group-hover:text-slate-300 transition-colors">คลิกเพื่ออัปโหลดรูปภาพ</span>
                            <span class="text-[10px] mt-1 opacity-60">หรือลากไฟล์มาวางที่นี่</span>
                        </div>

                        {{-- Input (Covering everything) --}}
                        <input type="file" name="thumbnail_file" id="thumbnail-input" accept="image/*"
                        ...
                        <div id="video-source-file"
                            ...
                            <div id="video-upload-container"
                                ...
                                <div class="flex flex-col items-center justify-center pointer-events-none">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                                        class="w-10 h-10 mb-2 text-slate-600 group-hover:text-emerald-500 transition-colors">
                                        <path
                            ...
                            @if($content->video_url && str_starts_with($content->video_url, '/storage'))
                                <div
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <p class="text-xs font-medium text-emerald-200 truncate">
                                            มีไฟล์วิดีโอเดิมอยู่แล้ว
                                        </p>
...
<style>
    ...
    /* Custom Font Size Labels: S, M, L, XL */
    ...
    /* ทำให้ toolbar ทึบขึ้นหน่อย */
    .ql-toolbar.ql-snow {
        background-color: rgba(15, 23, 42, 0.9);
        /* slate-900/90 */
    }

    /* พื้นหลัง editor ทึบขึ้นนิดหน่อย */
    .ql-container.ql-snow {
        background-color: rgba(2, 6, 23, 0.85);
        /* slate-950/85 */
    }

    /* สไตล์ปุ่ม picker (Normal, M, Heading ฯลฯ) */
    .ql-snow .ql-picker {
        color: #e5e7eb;
        font-size: 0.875rem;
    }
    ...
    /* กล่อง dropdown ของ picker (ที่มี Heading 1, Heading 2, Normal) */
    .ql-snow .ql-picker-options {
        ...
        z-index: 50;
        /* ดันขึ้นมาให้คลิกได้แน่นอน */
    }

    /* ตัวเลือกแต่ละอันใน dropdown */
    ...
    /* ให้ dropdown ของ picker (Heading / Normal / Size) ลอยบนหมด */
    .ql-snow .ql-picker-options {
        position: absolute;
        z-index: 40;
        background-color: rgba(15, 23, 42, 0.98);
    ...
    /* ลดระดับกล่องเนื้อหานิดนึงกันทับ dropdown */
    .ql-container.ql-snow {
        position: relative;
        z-index: 10;
    }
